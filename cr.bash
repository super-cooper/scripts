...
browser=open                                                # command to open your internet browser
...
id=$(git notes show $(git rev-parse $branch) 2> /dev/null | xargs echo)

# determines appropriate args based on whether or not an ID was found in git-notes
if [ -z $id ]; then
...
# submits the CR
output=$(source $venv && python3 $cr_upload ${other_args[@]} | tee /dev/tty | grep URL:)
code=$?
if [ $code -ne 0 ]; then
...
git notes add -m $id $(git rev-parse $branch) &> /dev/null

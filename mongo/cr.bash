...
cr_upload=$HOME/dev/mongodb/kernel-tools/codereview/upload.py   # path to the kernel-tools upload.py file
browser=vivaldi                                             # command to open your internet browser

if [ -n "$(git --git-dir=$HOME/dev/mongodb/enterprise/.git cherry -v)" ]; then
    yn=y
    if [ -n "$(git --git-dir=$HOME/dev/mongodb/mongo/.git cherry -v)" ]; then
    ...
    if [ $normalized == "Y" ] || [ $normalized == "YES" ]; then
        cd $HOME/dev/mongodb/enterprise
    fi
fi

# determine if this is a backport
upstream=$(git --git-dir=/home/adam/dev/mongodb/mongo/.git rev-parse --abbrev-ref "$branch@{upstream}" | sed 's/.*\///')
...
if [ -z "$id" ]; then
    ...
    other_args=( --email $email $@ -y --git_only_search_patch --git_similarity 75 --rev "$(git merge-base -a HEAD origin/$upstream)" )
    title="$(git log -1 --pretty=%B)"
else
    echo -e "\033[1;34mUpdating review...\033[0;0m"
    other_args=( $@ -y --git_only_search_patch --git_similarity 75 -i "$id" --rev "$(git merge-base -a HEAD origin/$upstream)" )
    title="$(git log -1 --pretty=%B | cut -d' ' -f2-)"
fi
...
# submits the CR
output=$(source "$venv" && python3 "$cr_upload" ${other_args[@]} --title "${title}")
